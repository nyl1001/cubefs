...
#get ticket for auth
./cfs-authtool ticket -host=172.16.0.178:8080 -keyfile=./authservice.json -output=./ticket_auth.json getticket AuthService
sleep 2s
#create admin
./cfs-authtool api -host=172.16.0.178:8080 -ticketfile=./ticket_auth.json -data=./data_admin.json -output=./key_admin.json AuthService createkey
#get ticket for admin
./cfs-authtool ticket -host=172.16.0.178:8080 -keyfile=./key_admin.json -output=./ticket_admin.json getticket AuthService
#create key for master
./cfs-authtool api -host=172.16.0.178:8080 -ticketfile=./ticket_admin.json -data=./data_master.json -output=./key_master.json AuthService createkey
#create key for client
./cfs-authtool api -host=172.16.0.178:8080 -ticketfile=./ticket_admin.json -data=./data_client.json -output=./key_client.json AuthService createkey
#create key for objectnode
./cfs-authtool api -host=172.16.0.178:8080 -ticketfile=./ticket_admin.json -data=./data_objectnode.json -output=./key_object.json AuthService createkey
